...
def extract_symbol_list_from_so(file_name: str) -> List[str]:
    """
    Extract list of symbols from ELF file.
    """
    try:
        ...
        print(f"nm -C {file_name} failed.")
    except OSError as exc:
        raise OSError("no usable 'nm' found, cannot continue") from exc
    return symlist
...
def extract_symbols_from_raw_list(rs):
    ...
    for sym in rs:
        ...
        assert sym_name, f"c++filt didn't demangle {sym}"
...
class SOInfo:
    ...
    fullname: str
    """
    The name of the library, as defined by GDB's :attr:`fullname` attribute.
    """

    symbols: bool
    """
    Whether the shared libary has symbols or not.
    """

    start: int
    """
    The address the shared library is mapped from.
    """

    end: int
    """
    The address the shared library is mapped to.
    """

    loaded: bool
    """
    Whether the symbols are loaded.
    """
...
class LoadLazySymbols(gdb.Command):
    ...
    def invoke(self, rec, from_tty):
        if rec == self.rec:
            ...
            gdb.execute(f"uload {rec}")


def ensure_frame_loaded(pc: int) -> bool:
    """
    Return
    """
    sodict = load_so_list()
        ...
        while curr_frame:
            try:
                ...
                if not curr_frame.name():
                    frames_loaded = ensure_frame_loaded(curr_frame.pc())
...
class ReverseStepSymbol(gdb.Command):
    ...
    def invoke(arg, from_tty):
        ...
        curr_frame = gdb.newest_frame()
        if not curr_frame.name():
            loaded = ensure_frame_loaded(curr_frame.pc())
            if not loaded:
                print(f"ERROR: shared library not found for address {curr_frame.pc():x}")
        curr_frame = gdb.newest_frame()
        # check symbol is now present
        if not curr_frame.name():
            print(f"WARNING: reverse step may be slow, no symbol found for PC {curr_frame.pc():x}")
...
class LoadLibFromSym(gdb.Command):
    """
    Command to load all libs that might define a symbol.
    """
    ...
    def check_lib_for_symbol(self, so: str, symbol: str) -> None:
        """
        TODO
        """
        ...
        for sym in rs:
            if symbol in sym:
                print(f"Got a match: {sym} contains {symbol}")
                match = True
        if match:
            ns = input("load library [y]/n ? ") or "y"
            if ns != "n":
                gdb.execute(f"sharedlibrary {so}")

    def load_libs_with_name(self, symbol: str) -> None:
        """
        TODO
        """
    ...
    def invoke(self, symbol: str, from_tty: bool) -> None:
        """
        TODO
        """
        if not self.sodict:
...
def time_gdb_execute(cmd: str) -> None:
    """
    Execute a GDB command, and print profiling information if enabled.
    """
    ...
    if PROFILING:
        print(f"Duration: {delta.total_seconds()} command: {cmd}")
